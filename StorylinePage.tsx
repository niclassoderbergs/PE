...
import { 
  ChevronLeft, ChevronRight, Info, Database, Network, Server, Cpu, CheckCircle2, Battery, Zap, UserCheck, Banknote, Truck
} from 'lucide-react';
...
const StoryStep: React.FC<{ step: Step; isActive: boolean; onClick: () => void }> = ({ step, isActive, onClick }) => {
  ...
  const getSystemLabel = (tech: string) => {
    if (tech.includes('FIS')) return 'FIS';
    if (tech.includes('DHV')) return 'DHV';
    if (tech.includes('BSP')) return 'BSP';
  ...
  return (
    ...
    >
      ...
      } w-72 h-[420px] mx-3 transition-colors`}>
        ...
        <div className={`mt-auto w-full p-4 rounded-xl text-xs transition-all ${
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <strong className="uppercase tracking-widest font-bold">
              {systemLabel === 'Extern process' ? 'Hantering' : (systemLabel === 'BSP' ? 'Aktör (BSP)' : `Systemet (${systemLabel})`)}
            </strong>
          </div>
...
export const StorylinePage: React.FC = () => {
  const [activeJourney, setActiveJourney] = useState<'elresa' | 'flexresa' | 'kvalificering' | 'affarsresa' | 'utflytt'>('elresa');
  ...
  const handleJourneySwitch = (journey: 'elresa' | 'flexresa' | 'kvalificering' | 'affarsresa' | 'utflytt') => {
  ...
  return (
    <div className="bg-slate-50 text-slate-900 pb-20 rounded-xl">
      <main className="max-w-7xl mx-auto px-6 pt-12">
        ...
        <div className="flex p-1 bg-gray-200/50 rounded-xl w-fit mb-12 shadow-inner">
          <button 
        ...
        <div className="max-w-2xl mb-12">
          ...
          <p className="text-lg text-slate-600 leading-relaxed">
            ...
            {activeJourney === 'kvalificering' && 'Denna resa visar de strikta tekniska stegen och valideringarna som sker i Flexregistret (FIS) för att säkra nätets driftsäkerhet.'}
            ...
            {activeJourney === 'utflytt' && 'När Kim flyttar ut ser systemen till att alla avtal och kopplingar städas bort automatiskt och säkert. Infrastrukturen sköter avvecklingen utan Kim behöver ringa runt.'}
          </p>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="p-8 bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-slate-500 leading-relaxed">Snabba byten och registreringar utan manuellt krångel genom automatiska IT-flöden for både el och flex.</p>
              </div>
